...
contract Osiris is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTotal = 1 * 10**6 * 10**9; // 1 million $OSIR Tokens in supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'osiris.holdings';
    string private _symbol = 'OSIR';
    ...
    constructor (address _presale) public {
        ...
        AVAXPair = IPangolinFactory(pangolinRouter.factory())
        .createPair(address(this), pangolinRouter.WAVAX());
        
       
        _isExcluded[presale] = true;
        _excluded.push(presale);
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256) {
        uint256 tFee = tAmount.mul(15).div(1000);
